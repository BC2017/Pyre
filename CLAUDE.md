...
- **geometry** — `Shape` trait, `Sphere`, `TriangleMesh`, `MeshInstance` (mesh + BVH + optional `InstanceMotion`), and our own SAH-binned `Bvh`. The `Shape` trait is what lights, integrators, and scene queries see. Motion-blurred instances keep a static object-space BVH; `MeshInstance::intersect` shifts the ray by `-motion.at(ray.time)` before lookup and translates the hit back — translation-only motion preserves `t` and `normal`.
...
- **camera** — `Camera` trait operating on a `CameraSample { ndc, lens, time }`. `PinholeCamera` (ignores `lens`) and `ThinLensCamera` (concentric-disk-sampled aperture, configurable focus plane). All cameras stamp `sample.time` onto the output ray so motion-blurred geometry resolves correctly through every bounce.
...
- **light** — `Light` trait + `DiffuseAreaQuadLight` for surface emitters; `EnvironmentLight` trait + `HdriEnvironmentLight` (equirectangular, `Distribution2D`-importance-sampled) for sky/HDRI. Surface lights live separately from primitives; shadow rays test only primitives. The environment lives on `Scene::env` (not in `Scene::lights`) because it's at infinity — it has no surface to intersect and is queried in two specific places: on miss, and as an extra NEE arm at every shading point.
- **integrator** — `PathIntegrator`: unidirectional path tracer with MIS direct lighting (NEE light-sample arm + BSDF-sample arm via the power heuristic) and Russian roulette. Returns a `PixelSample { radiance, albedo, normal, depth }`; the AOVs are captured on the first surface or light hit so downstream consumers (denoisers, comp tools) get them alongside the beauty pass.
- **film** — Stores one `PixelSample` per pixel (beauty + albedo/normal/depth AOVs). `save_png` writes the beauty as gamma-2.2 RGB; `save_exr` writes a single-layer multi-channel EXR with dotted-prefix channel names (`R`/`G`/`B` for beauty, `albedo.R/G/B`, `N.X/Y/Z`, `Z` for depth) — Nuke/Fusion auto-group those, OIDN reads `R/G/B`/`albedo`/`N` directly. `save_aov_pngs` writes per-AOV sidecar PNGs for quick inspection.
- **io** — `load_gltf` (walks the default scene's node hierarchy, bakes transforms into vertex data) and `load_pbrt` (hand-rolled lexer/parser/builder for a subset of PBRT v3/v4). The CLI dispatches on `--scene` file extension. A formal `SceneLoader` trait was deliberately not added — two free functions are cleaner for the current count, and the duplication cost is low; revisit if/when a third format lands.
...
- **6** — Three loosely related capabilities bundled because each is single-PR-sized on its own: HDRI environments, thin-lens DoF, motion blur. HDRI lives on `Scene::env: Option<Box<dyn EnvironmentLight>>` rather than in the `lights` Vec — env light geometry is "at infinity" with no surface to intersect, and the integrator queries it in two specific places that don't fit the surface-light hot path: on miss (with MIS against the BSDF arm) and as an extra NEE arm at every shading point (visibility via the new `Scene::occluded_dir`). Importance sampling is luminance × sin(theta) so the equirectangular Jacobian (`pdf_ω = pdf_uv / (2π² sin θ)`) cancels out; the `Distribution2D` helper in `distribution.rs` is reused for this and is the place to look first for future env / BSDF importance sampling. `HdriEnvironmentLight::gradient` / `::constant` provide procedural fallbacks so the studio preset works without an HDRI asset. `ThinLensCamera` is the Potmesil & Chakravarty thin-lens model — concentric-disk aperture sampling via `distribution::concentric_disk`, focus plane parallel to the image plane at `focus_distance`. The `Camera::generate_ray` signature was unified to take a single `CameraSample { ndc, lens, time }` rather than growing more positional parameters every milestone. Motion blur is translation-only on `MeshInstance` (`InstanceMotion { offset_t0, offset_t1 }`); the BVH stays static and `Shape::intersect` shifts the ray by `-motion.at(ray.time)` before lookup. `Scene::bounds()` now returns the swept world bounds (used by the auto-frame camera). Rotation/scaling motion, animated cameras, and animated lights are deferred to a follow-up. The integrator preserves `ray.time` across all bounces — one path = one shutter instant.
- **7** — Hand-rolled lexer + parser + builder for PBRT, split into three submodules of `io::pbrt`. The lexer is just enough — identifiers, strings, numbers (Int vs Float tokenized separately), brackets, `#`-comments. The parser is the only layer that knows about PBRT's directive shape (positional args + parameter pairs); detection is "string with whitespace = parameter header" because all PBRT type names and parameter names are unspaced identifiers. The builder owns the CTM stack and the attribute stack — `AttributeBegin/End` clones the current `Attribute { ctm, material_id, area_light }`. Sin-of-PBRT to watch for: parameter values can be written with bare integers even for float-class types (`"rgb L" [15 15 15]`), so the parser post-processes the value list and promotes `Int` arrays to `Float` arrays when `type_name` is in the float-class set (`float`, `rgb`, `point`, `vector`, `normal`, …). Without this, every accessor in `builder.rs` would need to handle both `Floats` and `Ints` independently. `AreaLightSource "diffuse"` only converts to a `DiffuseAreaQuadLight` when the following shape is a four-vertex two-triangle quad — anything else logs a warning and the emission is dropped (Pyre has no general surface emitter yet). Coordinate convention: pyre is +Y up; PBRT's spec is +Z up; the hand-authored `scenes/cornell.pbrt` is written for +Y up so it round-trips the built-in Cornell exactly. A `SceneLoader` trait was deferred (see `io` entry).
- **8** — AOVs (albedo, normal, depth) emitted by the integrator at the first hit, stored on the `Film` as `Vec<PixelSample>` rather than separate optional buffers — uniform 40-byte sample × w·h costs more memory than strictly necessary but eliminates the option-of-mutable-slice plumbing in the parallel render loop. Save sinks are independent of the render path: `save_png` is unchanged; `save_exr` writes a single-layer multi-channel EXR using `exr`'s `AnyChannels<FlatSamples>` with dotted channel names (`R/G/B`, `albedo.R/G/B`, `N.X/Y/Z`, `Z`). Two gotchas: (1) EXR requires alphabetically sorted channel names — sort before constructing the channel list or `to_file` fails with `channel names are not sorted alphabetically`; (2) area light hits use `albedo = Vec3::ONE` rather than the emission color, otherwise OIDN would over-correct toward bright sources. `Bsdf::albedo()` added to the trait (default `Vec3::ONE`, overridden by Lambertian and Disney). CLI: `--aovs albedo,normal,depth|all` opts in; `--save-aov-pngs` emits inspection-friendly sidecars (gamma-encoded albedo, `0.5·(n+1)` remapped normals, min-max normalised depth with closer = brighter).
